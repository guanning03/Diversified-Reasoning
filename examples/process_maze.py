...
def get_maze_prompt(maze: str) -> str:
    return f"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_train", action="store_true", help="跳过训练集处理")
    parser.add_argument("--no_test", action="store_true", help="跳过测试集处理")
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            question_raw = example.pop("content")

            question = get_maze_prompt(question_raw)

            # answer_raw = example.pop("answer")
            # solution = answer_raw
            solution = question_raw
            data = {
