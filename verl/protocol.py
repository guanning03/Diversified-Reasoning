...
class DataProto:
    ...
    def repeat(self, repeat_times=2, interleave=True, decoration = None):
        """
        ...
        if decoration is not None:
            assert isinstance(decoration, torch.Tensor), "Decoration must be a tensor"
            assert interleave, "Decoration is only supported for interleaved data"
        ...
        if decoration is not None:
            for i in range(len(repeated_non_tensor_batch['raw_prompt_ids'])):
                raw_prompt_ids = copy.deepcopy(repeated_non_tensor_batch['raw_prompt_ids'][i])
                # 找到所有匹配的索引
                cover_indices = [idx for idx, val in enumerate(raw_prompt_ids) if val == decoration[0].item()]
                # 如果匹配数量大于等于2，只保留第一个和最后一个
                if len(cover_indices) >= 2:
                    cover_indices = [cover_indices[0], cover_indices[-1]]
                # 替换匹配的位置
                for cover_idx in cover_indices:
                    raw_prompt_ids[cover_idx] = decoration[i % repeat_times].item()
                repeated_non_tensor_batch['raw_prompt_ids'][i] = raw_prompt_ids
            
            for i in range(len(repeated_batch['input_ids'])):
                # 找到所有匹配的索引位置
                cover_indices = (repeated_batch['input_ids'][i] == decoration[0]).nonzero(as_tuple=True)[0]
                # 如果匹配数量大于等于2，只保留第一个和最后一个
                if len(cover_indices) >= 2:
                    cover_indices = cover_indices[[0, -1]]
                # 替换匹配的位置
                for cover_idx in cover_indices:
                    repeated_batch['input_ids'][i][cover_idx] = decoration[i % repeat_times]
            print('✅ Prompt tuning decoration done!')
