...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if self._is_lora:
            assert self.config.model.num_lora_adapters > 0, f"Config crashed: lora_rank is {self._lora_rank} while num_lora_adapters is {self.config.model.num_lora_adapters}"
        else:
            assert self.config.model.num_lora_adapters == 0, f"Config crashed: lora_rank is {self._lora_rank} while num_lora_adapters is {self.config.model.num_lora_adapters}"
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self._is_lora:
                ...
                actor_module = get_peft_model(actor_module, LoraConfig(**lora_config), num_lora_adapters = self.config.model.num_lora_adapters)
        ...
        if rollout_name == "hf":
            ...
            lora_kwargs = (
                {"lora_kwargs": {"enable_lora": True, 
                                 "max_loras": self.config.model.num_lora_adapters, 
            ...
            rollout = vllm_rollout_cls(
                ...
                trust_remote_code=trust_remote_code,
                num_lora_adapters=self.config.model.get("num_lora_adapters", 0),
                **lora_kwargs,
            )
            ...
            rollout_sharding_manager = FSDPVLLMShardingManager(
                ...
                rollout_seed=self.config.rollout.get("seed", 1000),
                num_lora_adapters=self.config.model.get("num_lora_adapters", 0)
            )
    ...
    def generate_sequences(self, prompts: DataProto):
        # Support all hardwares
        # TODO: decorate prompt during rollout generation
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if self._is_lora and hasattr(getattr(self, "actor_module", self.actor_module_fsdp), "peft_config"):
            for i in range(1, self.config.model.num_lora_adapters + 1):
                lora_save_path = os.path.join(local_path, f"lora_adapter_{i}")
                peft_model = getattr(self, "actor_module", self.actor_module_fsdp)
                peft_config = {}
                if dist.get_rank() == 0:
                    os.makedirs(lora_save_path, exist_ok=True)
                    peft_config = asdict(peft_model.peft_config.get(f"LORA_{i}", {}))
                ...
                try:
                    if fsdp_version(self.actor_module_fsdp) > 0:
                        ...
                        lora_params = layered_summon_lora_params(self.actor_module_fsdp, f"LORA_{i}")
                        if dist.get_rank() == 0:
