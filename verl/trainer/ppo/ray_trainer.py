...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
    ...
    elif adv_estimator in [AdvantageEstimator.GRPO]:
        # Initialize the mask for GRPO calculation
        grpo_calculation_mask = data.batch["response_mask"]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns, pass_at_k_objectives = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        for key, value in pass_at_k_objectives.items():
            data.batch[key] = value
    elif adv_estimator in [AdvantageEstimator.PKPO, AdvantageEstimator.RLOO, AdvantageEstimator.GRPKPO]:
        ...
        if "uid" in data.non_tensor_batch.keys():  # optional
            adv_kwargs["index"] = data.non_tensor_batch["uid"]
        if "reward_baselines" in data.batch.keys():  # optional
            adv_kwargs["reward_baselines"] = data.batch["reward_baselines"]

        # calculate advantage estimator
        advantages, returns, pass_at_k_objectives = adv_estimator_fn(**adv_kwargs)
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        for key, value in pass_at_k_objectives.items():
            data.batch[key] = value
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.actor_rollout_ref.fixed_prompt_tuning.enable:
            self.decoration = torch.zeros(
                size = (len(self.config.actor_rollout_ref.fixed_prompt_tuning.role_prompts), ), 
                dtype=torch.int64
            )
            for i, role_prompt in enumerate(self.config.actor_rollout_ref.fixed_prompt_tuning.role_prompts):
                self.decoration[i] = torch.tensor(
                    self.tokenizer.encode(role_prompt), 
                    dtype=torch.int64
                )[0]
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_batch = test_batch.repeat(
                repeat_times=self.config.actor_rollout_ref.rollout.val_kwargs.n, interleave=True, decoration=self.decoration
            )
        ...
        metric_dict = guanning_compute_metrics(data_sources, 
                                               sample_scores, 
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if repeat_sampling_sglang_grpo:
                    assert self.decoration is None, "Decoration is not supported for SGLang rollout"
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if not torch.any(batch.batch["advantages"]):
                            skip_step = True
                    ...
                    # track stepwise KL after parameter update
                    with marked_timer("new_log_prob", timing_raw, color="blue"):
                        epsilon = 1e-10
                        old_log_prob = old_log_prob.batch["old_log_probs"]
                        new_log_prob = self.actor_rollout_wg.compute_log_prob(batch).batch["old_log_probs"]
                        response_masks = batch.batch["response_mask"]
                        old_probs = torch.exp(old_log_prob)
                        new_probs = torch.exp(new_log_prob)
                        old_probs = torch.clamp(old_probs, min=epsilon)
                        new_probs = torch.clamp(new_probs, min=epsilon)
                        old_log_prob = torch.log(old_probs)
                        new_log_prob = torch.log(new_probs)
                        log_prob_diff = new_log_prob - old_log_prob
                        masked_log_prob_diff = torch.masked_select(log_prob_diff, response_masks.bool())
                        stepwise_kl = torch.mean(masked_log_prob_diff)
                        metrics.update(
                            {
                                "training/stepwise_kl": stepwise_kl.detach().item()
                            }
                        )        
